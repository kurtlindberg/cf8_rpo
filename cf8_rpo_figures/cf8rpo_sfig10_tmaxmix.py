## Lake CF8 RPO Supplemental Figure 9
# Cross plots of RPO Tmax vs. MixSIAR-modeled OC endmember contributions
...
rpo_tmax_ens, rpo_tmax = cf8_fun.getlipd('Lindberg.CF8.2024.lpd',
...
tmax_mix = pd.DataFrame({'tmax':rpo_tmax.paleoData_values,
                         'mix_aqua':mix_aqua.paleoData_values,
                         'mix_post':mix_post.paleoData_values}
                        ).sort_values(by='tmax')

modelaqua = LinearRegression().fit(np.array(tmax_mix.tmax).reshape((-1,1)), np.array(tmax_mix.mix_aqua)*100)
modelpost = LinearRegression().fit(np.array(tmax_mix.tmax).reshape((-1,1)), np.array(tmax_mix.mix_post)*100)
...
ax = axs[0, 0]
ax.scatter(tmax_mix.tmax, tmax_mix.mix_post*100, s=40, color='orange', edgecolors='black', zorder=2,
           label="R = " + str(np.round(post_corr[0], decimals=3)) + "; p = " + str(np.round(post_corr[1], decimals=2)))
ax.plot(tmax_mix.tmax, (np.array(tmax_mix.tmax)*modelpost.coef_+modelpost.intercept_),
...
ax = axs[0, 1]
ax.scatter(tmax_mix.tmax, tmax_mix.mix_aqua*100, s=40, color='blue', edgecolors='black', zorder=2,
           label="R = " + str(np.round(aqua_corr[0], decimals=3)) + "; p = " + str(np.round(aqua_corr[1], decimals=2)))
ax.plot(tmax_mix.tmax, (np.array(tmax_mix.tmax)*modelaqua.coef_+modelaqua.intercept_),
        linestyle='--', color='black', zorder=1)
ax.set_xlim([375, 450])
ax.set_ylim([0, 100])
ax.set_xlabel('RPO Tmax (C)')
...
figures9 = plt.gcf()
# figures9.savefig('cf8rpo_figures9.svg')
